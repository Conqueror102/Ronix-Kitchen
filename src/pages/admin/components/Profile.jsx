          ...
          <div className="flex flex-col md:flex-row justify-between items-start md:items-center">
            <div>
              <h2 className="text-2xl font-bold text-black mb-2">
                Admin <span className="bg-clip-text text-transparent bg-vibrantOrange">Profile</span>
              </h2>
        ...
        <div className="bg-white/90 rounded-xl shadow-md overflow-hidden">
          <div className="p-6 flex flex-col items-center">
            <div className="relative">
              <div className="w-32 h-32 rounded-full overflow-hidden border-4 border-softOrange">
                <img 
                  src={profileImage} 
                  alt="Profile" 
              ...
              {isEditing && (
                <label className="absolute bottom-0 right-0 bg-vibrantOrange hover:bg-orange-600 text-white w-8 h-8 rounded-full flex items-center justify-center cursor-pointer shadow-lg transition-colors">
            ...
            <h3 className="mt-4 text-xl font-bold text-black">{profileData.name}</h3>
            <span className="px-3 py-1 mt-2 bg-softOrange/20 rounded-full text-vibrantOrange text-sm font-medium">
              {profileData.role}
            </span>
            
            <div className="mt-6 w-full space-y-4">
              <div className="flex items-center">
                <div className="w-8 h-8 rounded-full bg-softOrange flex items-center justify-center text-vibrantOrange mr-3">
                  <svg xmlns="http://www.w3.org/2000/svg" className="h-4 w-4" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                ...
                {isEditing ? (
                  <input 
                    ...
                    className="flex-1 bg-softOrange/40 text-black px-3 py-2 rounded-lg focus:outline-none focus:ring-2 focus:ring-vibrantOrange"
                  />
                ) : (
              ...
              <div className="flex items-center">
                ...
                <p className="text-gray-700">Joined: {profileData.joinDate}</p>
              </div>
            </div>
          </div>
        </div>
              ...
              <div className="space-y-4">
                <div>
                  ...
                  {isEditing ? (
                    ...
                    <p className="text-black bg-softOrange/20 px-4 py-3 rounded-lg">{profileData.name}</p>
                  )}
                </div>
                ...
                <div>
                  ...
                  {isEditing ? (
                    ...
                    <p className="text-black bg-softOrange/20 px-4 py-3 rounded-lg">{profileData.bio}</p>
                  )}
                </div>
              </div>
            </div>
              ...
              <div className="space-y-4">
                <div className="flex items-center justify-between">
                  <div>
                    ...
                    <p className="text-gray-600 text-sm">Receive notifications about orders and updates</p>
                  </div>
                  {isEditing ? (
                    <div 
                      onClick={() => handlePreferenceToggle('notifications')} 
                      className={`relative w-12 h-6 rounded-full transition-colors cursor-pointer ${
                        profileData.preferences.notifications ? 'bg-vibrantOrange' : 'bg-softOrange/40'
                      }`}
                    >
                      <span 
                ...
                <div className="flex items-center justify-between">
                  <div>
                    <h4 className="text-black font-medium">Marketing Emails</h4>
                    <p className="text-gray-600 text-sm">Receive promotional emails and offers</p>
                  </div>
                  {isEditing ? (
                    <div 
                      onClick={() => handlePreferenceToggle('marketingEmails')} 
                      className={`relative w-12 h-6 rounded-full transition-colors cursor-pointer ${
                        profileData.preferences.marketingEmails ? 'bg-vibrantOrange' : 'bg-softOrange/40'
                      }`}
                    >
                      <span 
                        className={`absolute top-0.5 left-0.5 h-5 w-5 rounded-full bg-white shadow-sm transform transition-transform ${
                ...
                <div className="flex items-center justify-between">
                  <div>
                    <h4 className="text-black font-medium">Two-Factor Authentication</h4>
                  ...
                  {isEditing ? (
                    <div 
                      ...
                      className={`relative w-12 h-6 rounded-full transition-colors cursor-pointer ${
                        profileData.preferences.twoFactorAuth ? 'bg-vibrantOrange' : 'bg-softOrange/40'
                      }`}
                    >
                      <span 
                        className={`absolute top-0.5 left-0.5 h-5 w-5 rounded-full bg-white shadow-sm transform transition-transform ${
