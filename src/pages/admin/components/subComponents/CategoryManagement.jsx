...
function CategoryManagement() {
    ...
    // Replace all categoryService calls with your backend API endpoints
    const fetchCategories = async () => {
        try {
            ...
            const fetchedCategories = [
                { id: 1, name: "Ramen", description: "Noodle soup", active: true, products: 5 },
                { id: 2, name: "Sushi", description: "Rice rolls", active: true, products: 3 }
            ];
    ...
    const onSubmit = async (data) => {
        try {
            ...
            if (isEditing && editingId) {
                // await fetch(`/api/categories/${editingId}`, { method: 'PUT', body: JSON.stringify(data) });
                setStatusMessage({ 
                    type: 'success', 
                    message: 'Category updated successfully' 
                });
            } else {
                // await fetch('/api/categories', { method: 'POST', body: JSON.stringify(data) });
                setStatusMessage({ 
                    type: 'success', 
              ...
              <div className="p-6">
                <div className="mb-4">
                  <h2 className="text-2xl font-bold text-black">
                    Category <span className="bg-clip-text text-transparent bg-vibrantOrange">Management</span>
                  </h2>
                            ...
                            <div className="space-y-4">
                                ...
                                <div className="flex justify-between pt-4">
                                    <button
                                        type="button"
                                        onClick={resetForm}
                                        className="px-4 py-2 text-sm font-medium text-white bg-black hover:bg-gray-900 rounded-md transition-colors"
                                        disabled={loading}
                                    >
                                        Cancel
                                    </button>
                                    <button
                                        type="submit"
                                        className="px-4 py-2 text-sm font-medium text-white bg-vibrantOrange hover:from-vibrantOrange hover:to-softOrange rounded-md transition-colors flex items-center"
                                        disabled={loading}
                                    >
                                        {loading && (
                                ...
                                <table className="min-w-full divide-y divide-softOrange/40">
                                    ...
                                    <tbody className="bg-transparent divide-y divide-softOrange/40">
                                        {filteredCategories.map((category) => (
                                            <tr key={category.id} className="hover:bg-softOrange/20 transition-colors">
                                                <td className="px-6 py-4 whitespace-normal break-words">
                                                    <div className="text-sm font-medium text-black">{category.name}</div>
                                                    <div className="text-sm text-gray-600">{category.description}</div>
                                                </td>
                                                <td className="px-6 py-4 whitespace-nowrap">
                                                    <div className="text-sm text-black">{category.products}</div>
                                                </td>
                                                <td className="px-6 py-4 whitespace-nowrap">
                                                    <span 
                                                ...
                                                <td className="px-6 py-4 whitespace-nowrap text-right text-sm font-medium">
                                                    <button 
                                                        ...
                                                        className="text-vibrantOrange hover:text-orange-600 mr-3"
                                                        disabled={loading}
                                                    >
                                                        Edit
                                                    </button>
